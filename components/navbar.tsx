...
const Navbar = () => {
  ...
  // Track active link for highlight
          ...
          <nav className="hidden lg:flex items-center space-x-12">
            {navLinks.map((link) => (
              <Link 
                key={link.name}
                href={link.href} 
                className={`nav-link text-base font-medium text-gray-200 hover:text-primary-gold relative transition-colors duration-200 ${activeLink === link.href ? 'border-b-[2px] border-primary-gold pb-1' : ''}`}
              >
                {link.name}
              </Link>
            ))}
            <button className="cta-primary bg-[#FFD700] text-[#000000] transform scale-[1.15]">Get Involved</button>
          </nav>
        </div>
      </header>
    );
              ...
              <nav className="flex flex-col space-y-4">
                {navLinks.map((link) => (
                  <Link 
                    key={link.name}
                    href={link.href} 
                    className={`text-gray-200 text-lg font-medium py-2 hover:text-primary-gold transition-colors duration-200 ${activeLink === link.href ? 'border-b-[2px] border-primary-gold pb-1' : ''}`}
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    {link.name}
                  </Link>
                ))}
                <button className="cta-primary mt-4 bg-[#FFD700] text-[#000000] transform scale-[1.15]">Get Involved</button>
              </nav>
            </div>
          </motion.div>
        )}
